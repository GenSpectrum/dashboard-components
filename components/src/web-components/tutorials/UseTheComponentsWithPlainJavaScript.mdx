...
For this tutorial, we use Vite as a build tool. You can use any other build tool, but this will not be covered in this tutorial.
...
<gs-app lapis="https://lapis.genspectrum.org/open/v2">
    ...
    <gs-number-sequences-over-time
        lapisFilters='[{ "displayName": "EG", "lapisFilter": { "pangoLineage": "EG*" }}, { "displayName": "JN.1", "lapisFilter": { "pangoLineage": "JN.1*" }}]'
        lapisDateField="date"
...
Now you can see that the `gs-number-sequences-over-time` component updates when you select a new location in the `gs-location-filter` component.
