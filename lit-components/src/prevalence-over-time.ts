...
export class PrevalenceOverTime extends LitElement {
  ...
  private fetchingTask = new Task(this, {
    task: async ([
      ...
      const fetchNumeratorQuery = new FetchAggregatedQuery<{ date: string | null }>(numerator, ['date']);
      const fetchDenominatorQuery = new FetchAggregatedQuery<{ date: string | null }>(denominator, ['date']);
      const mapNumeratorQuery = new MapQuery(fetchNumeratorQuery, (d) => mapDateToGranularityRange(d, granularity));
      const mapDenominatorQuery = new MapQuery(fetchDenominatorQuery, (d) => mapDateToGranularityRange(d, granularity));
  ...
  if (d.date !== null) {
    switch (granularity) {
      case 'day':
        ...
        break;
      case 'month':
        dateRange = `${new Date(d.date).getFullYear()}-${(new Date(d.date).getMonth() + 1).toString().padStart(2, "0")}`;
        break;
      case 'year':
